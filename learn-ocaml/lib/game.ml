...
let create ~height ~width ~initial_snake_length =
  ...
  | Some apple ->
    let t = { snake; apple; game_state = In_progress; score = 0; board } in
...
let handle_key t key =
  ...
  | Some k -> Snake.set_direction t.snake k
  | None -> ()
;;
...
let check_for_collisions t =
  if not (Board.in_bounds t.board (Snake.head t.snake)) then
...
let maybe_consume_apple t =
  ...
  then (
    ...
    | None -> t.game_state <- Win; t.score <- t.score + 1
